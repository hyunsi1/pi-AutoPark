...
class StateMachine:
    ...
    def _navigate_step(self, detections):
        ...
        MIN_MOVEMENT_THRESHOLD = 0.01  # 1cm 이상 움직였을 경우만 위치 갱신
        ...
        # FINAL_APPROACH 전환 조건 먼저 체크
        if dist_cmd >= MIN_MOVEMENT_THRESHOLD and dist < self.cfg.get("final_approach_threshold", 0.5):
            self.current_pos = target_pos
            self.logger.info("[NAVIGATE] FINAL_APPROACH 거리 도달 → 상태 전환")
            self.state = State.FINAL_APPROACH
            return

        # 일정 거리 이상 이동한 경우에만 위치 갱신
    ...
    def _wait_step(self):
        ...
        if not ret or frame is None:
            ...
            self.wait_start_time = now  # 다시 대기 시간 초기화
            return
    ...
    def _avoid_step(self):
        try:
            ...
            self.path = self.planner.replan_around(
                self.current_pos[:2], self.goal_slot[:2], obs_world,
