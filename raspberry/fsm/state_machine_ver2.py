...
class StateMachine:
    ...
    def _search_step(self, frame):
        self.wait_count = 0
        search_angles = [65, 90, 40]  # 전방, 왼쪽, 오른쪽 순으로 탐색
        found = False

        for angle in search_angles:
            # 서보모터 각도 조정
            self.ctrl.set_angle(angle)
            self.logger.info(f"[SEARCH] Trying angle {angle} deg")
    
            # 약간의 딜레이 (서보모터가 움직이는 시간 보장)
            time.sleep(0.2)
    
            # 실제 탐색: 기존 로직을 함수로 빼서 재사용
            if self._find_parking_goal(frame):
                found = True
                break  # 목표 찾으면 반복 종료
    
        if not found:
            self.state = State.SEARCH  # 그래도 못 찾으면 다시 SEARCH로 돌아가기
            
    def _find_parking_goal(self, frame):
